...
// Когда работало и делался 1 запрос в 1,5 сек
...
// Адаптивный график — расширяем окно если мало точек
...
async function fetchAndSaveStats(limit = 100) {
  ...
  console.log(`[StatsService] Обрабатываем ${skins.length} скинов`);
  ...
  for (let i = 0; i < skins.length; i++) {
    ...
    try {
      const { data } = await steamService.fetchPriceHistory(skin.market_hash_name);
    ...
    if (i < skins.length - 1) await delay(DELAY_MS);
  }
